...
if (!$result = $connect->query($query)) {
  if ($email == NULL) {
    die('You forgot to fill out the form! <a href="reset.php?f=password">Click Here</a> to go back');
  } else {
  die('There was an error, sorry! <a href="reset?f=password">Click Here</a> to go back, ' . $email . '');
  }
}
...
$last_pass_reset = time();

$message = "
Hello, " . $email . ".
A failed attempt to log in to your Streampost account resulted in a request for password reset.
If you did not request this, please take the time to secure your account by resetting the password
and ensuring that your information has not been compromised.

To reset your password, please visit the following link: http:// " . $db_url . "/admin/users/pass-reset?id=" . md5($username) . "&init=" . $last_pass_reset;
...
if ($match) {
  mail($email, 'Forgotten Password', $msg, $headers);
}
      ...
      <div class='row deep-orange'>
        <div class='col s12 m8 offset-m2'>
          <h4>
            ...
            <a href='reset?f=password' class='black-text'>Click here</a> to go back
          </h4>
        </div>
      <?php } else { ?>
      <div class='row green'>
